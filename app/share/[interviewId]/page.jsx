...
export default function SharePage({ params }) {
  ...
  if (loading) {
    return (
      <DarkShell className="flex min-h-screen items-center justify-center">
        <div className="h-10 w-10 animate-spin rounded-full border-4 border-[#22d3ee]/30 border-t-[#22d3ee]" />
      </DarkShell>
    );
  }
  ...
  if (notFound) {
    return (
      <DarkShell className="flex min-h-screen flex-col items-center justify-center gap-4 p-8 text-center">
        <Sparkles className="h-12 w-12 text-[#22d3ee]" />
        <h1 className="text-2xl font-bold text-white">Feedback not found</h1>
        <p className="max-w-sm text-[#22d3ee]">
          This link may be invalid or the interview hasn&apos;t been completed yet.
        </p>
        <Link
          href="/"
          className="mt-2 inline-flex items-center gap-1.5 rounded-full bg-[#22d3ee] px-5 py-2 text-sm font-semibold text-[#06180d] hover:bg-[#a855f7]"
        >
  ...
  return (
    <DarkShell className="min-h-screen">
      {/* Top bar */}
      <header className="sticky top-0 z-20 border-b border-[#22d3ee]/15 bg-[#071109]/80 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-4xl items-center justify-between px-4 sm:px-6">
          <Link href="/" className="flex items-center gap-2">
            <div className="flex h-8 w-8 items-center justify-center rounded-xl bg-[#22d3ee]/15">
              <Sparkles className="h-4 w-4 text-[#22d3ee]" />
            </div>
            <div className="flex flex-col">
              <span className="font-black text-[#f5f0e8] tracking-tight leading-tight">
                Career<span className="text-[#22d3ee]">AI</span>
              </span>
              <span className="text-[10px] font-medium text-[#c084fc] leading-tight">
          ...
          <div className="flex items-center gap-3">
            <button
              onClick={copyLink}
              className="flex items-center gap-1.5 rounded-full border border-[#22d3ee]/30 bg-[#0b1c12] px-4 py-1.5 text-sm font-medium text-[#22d3ee] shadow-sm transition hover:border-[#22d3ee]/60 hover:text-[#a855f7]"
            >
              {copied ? (
            ...
            <Link
              href="/"
              className="flex items-center gap-1.5 rounded-full bg-[#22d3ee] px-4 py-1.5 text-sm font-semibold text-[#06180d] transition hover:bg-[#a855f7]"
            >
      ...
      <main className="mx-auto max-w-4xl space-y-6 px-4 py-10 sm:px-6">
        {/* Hero card */}
        <div className="overflow-hidden rounded-2xl border border-[#22d3ee]/20 bg-gradient-to-br from-[#12351f] via-[#0d2a18] to-[#071109] shadow-xl">
          <div className="p-6 md:p-8">
            <p className="text-xs font-semibold uppercase tracking-widest text-[#c084fc]">
              Mock Interview Results
            </p>
            <h1 className="mt-2 text-3xl font-black text-white">
              {interview.jobPosition}
            </h1>
            <div className="mt-2 flex flex-wrap gap-4 text-sm text-[#22d3ee]">
          ...
          <div className="flex flex-wrap items-center gap-8 bg-black/25 px-6 py-5 md:px-8">
            ...
            <div>
              <p className="text-sm font-medium text-[#22d3ee]">Overall score</p>
        ...
        <div className="space-y-3">
          {feedbackList.map((item, index) => (
            <Collapsible key={index} className="overflow-hidden rounded-2xl border border-[#22d3ee]/15 bg-[#0d2a18]/80 shadow-lg shadow-black/30 backdrop-blur">
              <CollapsibleTrigger className="flex w-full items-center justify-between gap-4 p-4 text-left hover:bg-[#0b1c12] transition-colors">
                <div className="flex min-w-0 items-center gap-3">
                  <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-[#22d3ee]/15 text-xs font-bold text-[#22d3ee]">
                    {index + 1}
                  </span>
                ...
                <div className="flex shrink-0 items-center gap-2">
                  ...
                  <ChevronsUpDownIcon className="h-4 w-4 text-[#c084fc]" />
                </div>
              </CollapsibleTrigger>
              <CollapsibleContent>
                <div className="space-y-2 border-t border-[#22d3ee]/15 bg-[#08160d] p-4">
        ...
        {/* CTA banner */}
        <div className="rounded-2xl border border-[#22d3ee]/20 bg-[#0b1c12] p-6 text-center">
          <Trophy className="mx-auto h-8 w-8 text-[#22d3ee]" />
          ...
          <Link
            href="/"
            className="mt-4 inline-flex items-center gap-2 rounded-full bg-[#22d3ee] px-6 py-2.5 text-sm font-semibold text-[#06180d] shadow hover:bg-[#a855f7] transition"
          >
            Start practicing now <ArrowRight className="h-4 w-4" />
          </Link>
        </div>

        <p className="text-center text-xs text-[#c084fc]">
